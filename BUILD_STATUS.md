...
Built and verified end-to-end on the `~/Valet/wp` WP 7.0 install. The plugin can now
...
Tested live on the `~/Valet/wp` WP 7.0 install (`http://wp.test`) with Anthropic
...
Symlinked into `~/Valet/planner/htdocs/wp-content/plugins/` (a real **WP 7.0** install)
...
A complete Phase 1 plugin at `/Users/filip/Projects/wp-ai-rate-limiter/`:
