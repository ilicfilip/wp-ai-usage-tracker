...
Background docs live outside the repo in `~/Assistant/Online Profiles/`:
...
There is a real WP 7.0 Valet install at `~/Valet/wp` (URL `http://wp.test`, admin
`filip` / `pass`) with the Anthropic provider configured. The plugin is symlinked into
