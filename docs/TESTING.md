...
(`/Users/filip/Projects/wp-ai-rate-limiter`).
...
- **Install root:** `~/Valet/wp` (Laravel Valet).
- **Symlink:** the repo is symlinked into
  `~/Valet/wp/wp-content/plugins/wp-ai-rate-limiter` — so edits in
  `/Users/filip/Projects/wp-ai-rate-limiter` are live immediately; no copy step.
...
```bash
wp --path=$HOME/Valet/wp db query "SHOW TABLES LIKE 'wp_aiut_%'"
...
```bash
wp --path=$HOME/Valet/wp db query \
...
- Alerts **dedup per (limit, period, threshold)** via a transient
  ...
  `wp --path=$HOME/Valet/wp transient delete --all` (or delete the specific
...
wp --path=$HOME/Valet/wp db query "TRUNCATE wp_aiut_events"
wp --path=$HOME/Valet/wp db query "TRUNCATE wp_aiut_counters"
wp --path=$HOME/Valet/wp db query "TRUNCATE wp_aiut_limits"
# Clear the cached hard-limit flag + alert dedup transients:
wp --path=$HOME/Valet/wp option delete aiut_has_hard_limits
wp --path=$HOME/Valet/wp transient delete --all
```
...
2. **Dashboard rebuilt** if you changed `assets/src/**`: `npm run build`,
   ...
   http://wp.test and confirm it mounts (no console errors, REST calls to
