 ...
 * Plugin URI:        https://github.com/filipilic/wp-ai-rate-limiter
 ...
 * Text Domain:       wp-ai-rate-limiter
 *
 ...
 * Plugin version.
 */
define( 'WP_AI_RATE_LIMITER_VERSION', '0.1.0' );

/**
 * Absolute path to the main plugin file.
 */
define( 'WP_AI_RATE_LIMITER_PLUGIN_FILE', __FILE__ );

/**
 * Absolute path to the plugin directory, with trailing slash.
 */
define( 'WP_AI_RATE_LIMITER_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );

/**
 * URL to the plugin directory, with trailing slash.
 */
define( 'WP_AI_RATE_LIMITER_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
...
/**
 * Autoloader for the \WP_AI_Rate_Limiter\ namespace, using the WordPress
 ...
 * class-prefixed file, so \WP_AI_Rate_Limiter\Capture\Gatekeeper resolves to
 * src/Capture/class-gatekeeper.php and \WP_AI_Rate_Limiter\Data\Schema resolves
 * to src/Data/class-schema.php.
 *
 * @param string $class Fully qualified class name.
 * @return void
 */
function wp_ai_rate_limiter_autoload( $class ) {
	$prefix = 'WP_AI_Rate_Limiter\\';
	$len    = strlen( $prefix );
	...
	$path     = WP_AI_RATE_LIMITER_PLUGIN_DIR . 'src/' . $sub_path . $file_name;

	if ( is_readable( $path ) ) {
		require_once $path;
	}
}
spl_autoload_register( 'wp_ai_rate_limiter_autoload' );
 ...
 */
function wp_ai_rate_limiter_environment_ok() {
	global $wp_version;
 ...
 * @return void
 */
function wp_ai_rate_limiter_activate() {
	if ( ! wp_ai_rate_limiter_environment_ok() ) {
		// Refuse to activate: deactivate ourselves and flag a notice.
		deactivate_plugins( plugin_basename( WP_AI_RATE_LIMITER_PLUGIN_FILE ) );
		set_transient( 'wp_ai_rate_limiter_activation_error', 1, 60 );
		return;
	}
...
register_activation_hook( __FILE__, 'wp_ai_rate_limiter_activate' );
 ...
 * @return void
 */
function wp_ai_rate_limiter_activation_notice() {
	if ( ! get_transient( 'wp_ai_rate_limiter_activation_error' ) ) {
		return;
	}

	delete_transient( 'wp_ai_rate_limiter_activation_error' );
...
add_action( 'admin_notices', 'wp_ai_rate_limiter_activation_notice' );
 ...
 * @return void
 */
function wp_ai_rate_limiter_boot() {
	if ( ! wp_ai_rate_limiter_environment_ok() ) {
		return;
	}

	if ( class_exists( '\\WP_AI_Rate_Limiter\\Plugin' ) ) {
		\WP_AI_Rate_Limiter\Plugin::boot();
	}
}
add_action( 'plugins_loaded', 'wp_ai_rate_limiter_boot' );
